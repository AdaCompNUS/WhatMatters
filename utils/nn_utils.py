...
class Regressor:
    ...
    def infer_map_toplevel(
        ...
        """Test the model on different prediction horizons for map based Nearest Neighbors.
        ...
        pred_trajectories = baseline_utils.merge_saved_traj(temp_save_dir, args.traj_save_path)
        shutil.rmtree(temp_save_dir)

        return pred_trajectories

    def infer_absolute_toplevel(
            self,
            train_input: np.ndarray,
            train_output: np.ndarray,
...
class MopedRegressor(Regressor):
    def train_map(
            self,
            train_input: np.ndarray,
            train_output: np.ndarray,
        ...
        grid_search = self.run_grid_search(train_input, train_output_curr)
        print("Model obtained, Done")
    ...
    def train_absolute(
        ...
        # Run experiments over different prediction horizons
        # Flatten from (num_tracks x pred_horizon x feature_size) to (num_tracks x feature_size)
        train_output_curr = train_output[:, :pred_horizon, :].reshape(
            (train_num_tracks, pred_horizon * 2), order="F")
    ...
    def val_map(self, grid_search: GridSearchCV,
                  train_output: np.ndarray,
                  val_helpers: pd.DataFrame,
                  num_features: int,
                  args: Any) -> Dict[str, List[np.ndarray]]:

        pred_horizon=30
        test_centerlines = val_helpers["CANDIDATE_CENTERLINES"].values
        test_nt = val_helpers["CANDIDATE_NT_DISTANCES"].values
        test_references = val_helpers["CANDIDATE_DELTA_REFERENCES"].values
        test_seq_ids = val_helpers["SEQUENCE"].values

        test_num_tracks = test_nt.shape[0]

        print(f"Map-based Inference currently")

        forecasted_trajectories = {}

        # Predict for each trajectory
        for i in tqdm(range(test_num_tracks)):
            ...
            for (
                ...
                y_pred = train_output[neigh_idx][
                         0, :, :pred_horizon, :]  # num_neighbors x curr_pred_len x 2
                test_input = np.repeat(test_input,
                                       repeats=args.n_neigh,
                                       axis=0)
                ...
                # Using only 1 candidate to predict, thus breaking here
                break
    ...
    def infer_map_for_planner(self, grid_search: GridSearchCV,
                train_output: np.ndarray,
                val_helpers: pd.DataFrame,
                num_features: int,
                args: Any) -> Dict[str, List[np.ndarray]]:
    ...
    def val_absolute(self, grid_search: GridSearchCV,
                train_output: np.ndarray,
                test_input: np.ndarray,
                test_helpers: pd.DataFrame,
                num_features: int,
                args: Any) -> Dict[str, np.ndarray]:
        ...
        print(f"Absolute Inference currently")
        ...
        for i in tqdm(range(test_num_tracks)):
            ...
            curr_test_input = np.repeat(test_input[i],
                                        ...
                                        axis=0)
            neigh_output = train_output[neigh_idx][
                           i, :, :pred_horizon, :]  # num_neigh x curr_pred_len x 2
