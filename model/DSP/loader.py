...
#
from model.DSP.networks.dsp import DSP
from model.DSP.utils.dataset import DspArgoDataset
from model.DSP.utils.loss_fn import LossFn
from model.DSP.utils.optimizer import Optimizer
from model.DSP.utils.evaluator import TrajPredictionEvaluator
...
class Loader:
    ...
    def __init__(self, mode, feature_dir, adv_cfg_path, model, loss, device, is_ddp=False, world_size=1, local_rank=0):
        self.mode = mode
        self.data_aug = False
        self.features_dir = feature_dir
        self.adv_cfg_path = adv_cfg_path
        self.model = model
        self.loss = loss
        self.device = device
        self.is_ddp = is_ddp
        self.adv_cfg = import_module('{}'.format(self.adv_cfg_path)).AdvCfg(is_ddp=is_ddp)
    ...
    def get_dataset(self):
        train_dir = self.features_dir + 'train/'
        val_dir = self.features_dir + 'val/'
        test_dir = self.features_dir + 'test_obs/'
