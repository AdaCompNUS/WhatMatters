...
def process_lane_data(
    ...
    if dataset == 'summit':
        radius: float = max(10, agent_velocity_vector_length * radius_scale)
    else:
        radius: float = agent_velocity_vector_length * radius_scale
    # print(f"the lane radius is {radius}")
    ...
    for lsi in lane_segment_ids:
        ...
        if dataset == "summit":
            temp = np.zeros([10, 2])
            f = interpolate.interp1d(range(len(centerline)), centerline, axis = 0)
            for i in range(10):
                range_new = i*(len(centerline)-1)/(10-1)
                temp[i] = f(range_new)
            centerline = temp
...
class ArgoverseHDPipeline(pipeline.Pipeline):
    ...
    def process(self, data: Any) -> Any:
        ...
        try:
            ...
            #print(f"Number of candidate centerline: {centerline_candidate_features.shape[0]}")
...
class ArgoverseForecastingLoaderWrapper:
    ...
    def __iter__(self):
        for data in self._avfl:
            sequence_name = os.path.basename(data.current_seq).split('.')[0]
...
def run(config: config_parser.GlobalConfig):
    ...
    for split_name in conventions.SPLIT_NAMES:
        ...
        ds_path = os.path.join(datasets_path, split_name, 'data')
