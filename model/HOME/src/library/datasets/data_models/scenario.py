...
class ScenarioData:
    ...
    def load(cls, path: str) -> 'ScenarioData':
        ...
        if filename.split('_')[0] in ["magic", "beijing", "highway", "MIA", "PIT"]:
            sequence_city = filename.split('_')[0]
        elif filename.split('_')[0] in ["chandni", "meskel"]:
            sequence_city = "_".join(filename.split('_')[:2])
        else:
            sequence_city = "_".join(filename.split('_')[:4])
        sequence_id = filename[len(sequence_city)+1:]
