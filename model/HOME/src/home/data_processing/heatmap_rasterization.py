...
# def is_point_connected(driveable_area: np.ndarray, x_center: int, y_center: int, true_driveable_area_value: int) -> bool:
#     visited = np.zeros(driveable_area.shape, dtype=np.bool)
#     stack = [(x_center, y_center)]

#     while stack:
#         x, y = stack.pop()
#         if not (0 <= x < driveable_area.shape[0] and 0 <= y < driveable_area.shape[1]) or visited[x, y]:
#             continue

#         visited[x, y] = True

#         if driveable_area[x, y] == true_driveable_area_value:
#             return True
...
#     true_driveable_area_value: int = 1
# ) -> np.ndarray:  
...
#     driveable_area[up:bottom, left:right] = 1
...
#     # Modify driveable_area to ensure connectivity
#     dilation_kernel = np.ones((dilation_kernel_size, dilation_kernel_size), np.uint8)
    
#     while not is_point_connected(driveable_area, x_center, y_center, true_driveable_area_value):
#         driveable_area = cv2.dilate(driveable_area, dilation_kernel, iterations=1)

#     heatmap = heatmap * driveable_area[0]  # Set probability to 0 in non-driveable area
...
def create_heatmap(
    ...
    heatmap[up:bottom, left:right] = 1
    if dataset == 'summit':
        driveable_area[0][up:bottom, left:right] = 1
