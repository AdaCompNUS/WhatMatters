...
def evaluate(
    ...
    with torch.no_grad():
        for scenario_index in tqdm(range(len(dataset))):
            ...
            # add new matrix here
            agent_ade = metrics.ADE(forecasts_scaled[0], gt_traj_scaled)
            agent_fde = metrics.FDE(forecasts_scaled[0], gt_traj_scaled)
            agent_ade = agent_ade.detach().item()
            agent_fde = agent_fde.detach().item()
            if agent_fde > 2.0:
                misses += 1
            ...
            logger.debug(f'[Current-Stats]: MissRate6={n_misses / completed_scenarios}')
            logger.debug(f'[Current-Stats]: ADE={total_agent_ade / completed_scenarios}')
            logger.debug(f'[Current-Stats]: FDE={total_agent_fde / completed_scenarios}')
            logger.debug(f'[Current-Stats]: MissRate={misses / completed_scenarios}')

        dataset_metrics = {
            'agent-mean-minADE': total_agent_min_ade / n_scenarios,
            'agent-mean-minFDE': total_agent_min_fde / n_scenarios,
            'agent-mean-ADE': total_agent_ade / n_scenarios,
            'agent-mean-FDE': total_agent_fde / n_scenarios
        }
