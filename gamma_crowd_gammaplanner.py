            ...
            if get_velocity(agent.actor).length() < c.args.stuck_speed:
                if agent.stuck_time is not None:
                    if update_time - agent.stuck_time >= c.args.stuck_duration:
                        ...
                        print('Agent {} stuck speed {}, duration {} < {}.'.format(agent.actor.id,
                    get_velocity(agent.actor).length(), update_time - agent.stuck_time, c.args.stuck_duration))
...
def do_control(c, speed_pid_integrals, speed_pid_last_errors, steer_pid_integrals, steer_pid_last_errors, pid_last_update_time):
    ...
    if pid_last_update_time is None:
        ...
        # I change dt here so that even with bigger control rate, I do not need to update PID gains
        dt = min(dt, 1.0/ORIGINAL_CONTROL_MAX_RATE)
    
    just_for_print_one_agent = True
    for (actor_id, type_tag, control_velocity, preferred_speed, steer_angle_range) in control_velocities:
        # print("TEMP: actor_id: ", actor_id, "type_tag: ", type_tag, "control_velocity: ", 
        #       control_velocity, "preferred_speed: ", preferred_speed, "steer_angle_range: ", steer_angle_range)
        ...
        if type_tag == 'Car' or type_tag == 'Bicycle':
            ...
            # Update the PID gains to adjust the frequency. Work at 300, 50 slower. Testing at 900 slower
            freq_ratio = CONTROL_MAX_RATE / ORIGINAL_CONTROL_MAX_RATE
            # speed_kp = speed_kp * freq_ratio
            # speed_ki = speed_ki * freq_ratio
            # speed_kd = speed_kd * freq_ratio
            # steer_kp = steer_kp * freq_ratio
            # steer_ki = steer_ki * freq_ratio
            # steer_kd = steer_kd * freq_ratio
            ...
            # if just_for_print_one_agent:
            #     print("speed_control first: ", speed_control, "speed kp: ", speed_kp, "speed ki: ", speed_ki,
            #            "speed kd: ", speed_kd, "steer ", steer_control)
            ...
            # if just_for_print_one_agent:
            #     print("speed_control second: ", speed_control, " steer ", steer_control)
            ...
            # if just_for_print_one_agent:
            #     just_for_print_one_agent = False

            #     print("ID", actor_id, "freq", freq_ratio, "dt", round(dt,2), "speed ", speed, "target_speed", round(target_speed,2), "speed_error: ", round(speed_error,2), "speed_control: ",
            #                 round(speed_control,5), "throttle: ", round(control.throttle,5),
            #                 "speed pid", speed_pid_integrals[actor_id], "len acts: ", len(c.world.get_actors()))
            #     print("ID", actor_id, "heading ", heading, "target_heading", round(target_heading,2), "control_steer: ", round(control.steer,2))
...
def spawn_destroy_loop(args):
    try:
        ...
        print('Spawn-destroy slow simulation loop running.')
        ...
        while True:
            ...
            #print('({}) Spawn-destroy time spent: {} Hz'.format(os.getpid(), time.time() - start))
...
def control_loop(args):
    try:
        ...
        time.sleep(3)
        c = Context(args)
        print('Control loop slow simulation running.')
        ...
        # Do not control when does carla_tick does not enter main While
        # while True:
        #     start = time.time()
        #     time.sleep(0.1)

        #     if (len(c.world.get_actors()) >= (args.num_car + args.num_pedestrian + args.num_bike)/1.2) \
        #             or (time.time() - start > 15.0):
        #         break

        #     print('Do not control now because num actor: ', len(c.world.get_actors()),' / ', args.num_car + args.num_pedestrian + args.num_bike)
        ...
        while True:
            ...
            # I put this so that crowd_service has attentions to publish by crowd_processor as gamma_rate runs slower than crowd_processor
            # if not c.world.get_settings().synchronous_mode:
            #     time.sleep(max(0.0, 1 / ORIGINAL_CONTROL_MAX_RATE - (time.time() - start))) # 20 Hz
            #     continue


            time.sleep(max(0.0, 1 / CONTROL_MAX_RATE - (time.time() - start))) # 20 Hz
            #print('({}) Control rate: {} Hz'.format(os.getpid(), 1 / max(time.time() - start)))
    except Pyro4.errors.ConnectionClosedError:
        pass
...
def gamma_loop(args):
    try:
        ...
        time.sleep(3)
        c = Context(args)
        print('GAMMA slow simulation loop running.')
        ...
        while True:
            ...
            # I put this so that crowd_service has attentions to publish by crowd_processor as gamma_rate runs slower than crowd_processor
            # if not c.world.get_settings().synchronous_mode:
            #     time.sleep(max(0.0, 1 / ORIGINAL_GAMMA_MAX_RATE - (time.time() - start))) # 40 Hz
            #     continue


            time.sleep(max(0.0, 1 / GAMMA_MAX_RATE - (time.time() - start))) # 40 Hz
            #print('({}) GAMMA rate: {} Hz'.format(os.getpid(), 1 / (time.time() - start)))
...
def carla_tick(args):
    ...
    _old_settings = world.get_settings()

    try:
        print('carla_tick loop slow-sim constacc running.')
        start = time.time()

        settings = world.get_settings()
        settings.fixed_delta_seconds = 1.0 / 30 # Running 30 Hz per tick
        ...
        while True:
            start = time.time()
            world.tick()
            time.sleep(max(0.0, 1.0 / 30 - (time.time() - start))) # Tick 30 Hz as original

            if (len(world.get_actors()) >= (args.num_car + args.num_pedestrian + args.num_bike)/1.2) \
                    or (time.time() - start > 15.0):
                break

            print('Current number of actors ', len(world.get_actors()),
                  ' waiting for ', args.num_car + args.num_pedestrian + args.num_bike)
        ...
        while True:
            ...
            world.tick()
            #world.wait_for_tick()
            time.sleep(max(0.0, 1.0 / SUMMIT_TICK_FREQUENCY_RATE - (time.time() - start))) # 10 times slower so 0.03 increase to 0.3 seconds of sleeping
            #print('({}) Tick rate: {} Hz, time {}'.format(os.getpid(), 1 / (time.time() - start),
            #                                              time.time() - start))
            print('Current number of actors ', len(world.get_actors()), ' / ', args.num_car + args.num_pedestrian + args.num_bike)
    
    except Pyro4.errors.ConnectionClosedError:
        print('Exit the simulator3')
    except Exception as e:
        print('Exception in carla_tick loop: {}'.format(e))
        print('Exit the simulator2')
    finally:
        # Turn to asynchronous mode so that any thing futher can destroy the simulator
        print('Exit the simulator')
        #world.tick()
        world.apply_settings(_old_settings)

def main(args):

    carla_tick_proc = Process(target=carla_tick, args=(args,))
    carla_tick_proc.daemon = True
    carla_tick_proc.start()
