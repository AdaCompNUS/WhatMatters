...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None):
        ...
        if map_inst:
            if isinstance(map_inst, carla.SumoNetwork):
                self._map = map_inst
            else:
                assert False, "map_inst must be a carla.SumoNetwork instance"
        else:
            assert False, "map_inst must be a carla.SumoNetwork instance"
    ...
    def _init_controller(self):
        ...
        current_routepoint = self._map.get_nearest_route_point(position2d) # A routpoint, not waypoint

        self.target_waypoint, self.target_road_option = (current_routepoint, RoadOption.LANEFOLLOW)
