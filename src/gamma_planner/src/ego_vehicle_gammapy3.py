...
EGO_VEHICLE_UPDATE_FREQUENCY_IN_HZ = 20.0 / 6 # 5Hz thus divide by 6
ORIGINAL_EGO_VEHICLE_PUBLISH_INFO_FREQUENCY_IN_TIME = 0.02 #0.02 # Updating every 0.5 seconds -> Hz = 2, if 10 times slower, use 0.2

CONTROL_FREQ = 3.0 
PRINT_LOG = False

## Slowing down requires 3 changes
# 1/ timescale in run_data_collection.py
# 2/ this file ego_vehicle_update
# 3/ summit sim to reduce stuck time
# 4/ gamma_crowd
    ...
    def update_gamma_control(self, step = None):
        ...
        for (i, actor) in enumerate(self.world.get_actors()):
            ...
            if actor.id in agent_prediction: # The most near 20 agents are predicted
                ...
                delta_t = 0.3 # We assume each prediction is distanced by 0.3 seconds. It should regarldess of we slow down
