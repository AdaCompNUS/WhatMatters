...
def validate(
    ...
    cprint(
                f"Val -- Epoch:{epoch}, loss:{val_loss}, Rollout: {rollout_len}",
                color="green",
            )
    middle_dir = "saved_models_summit" if dataset == "SummitDataset" else "saved_models"
    if val_loss <= best_loss:
        best_loss = val_loss
        save_dir = "model/LSTM/" + middle_dir + "/lstm_map"
...
def infer_map(
    ...
    for i, (_input, target, helpers) in enumerate(test_loader):
        ...
        for batch_idx in range(batch_size):
            ...
            seq_id = helpers_dict["SEQ_PATHS"][batch_idx]
            abs_outputs = []
            ...
            for candidate_idx in range(num_candidates):
                ...
                _input = torch.FloatTensor(
                    np.expand_dims(curr_nt_dist[:obs_len].astype(float),
                                   0)).to(device)
                ...
                decoder_outputs = torch.zeros((1, pred_len, 2)).to(device)
...
def main(train_features, val_features, obs_len, pred_len, normalize, use_delta, use_map, use_social, lr, cfg, test, dataset):
    ...
    if dataset == "SummitDataset" and test == False:
        ...
        #data_dict = baseline_utils.get_data(obs_len, pred_len, use_map, normalize, use_delta, baseline_key, train_features, val_features)
    else:
        args = Config(dict(
            use_map=use_map,
            use_social=use_social,
        ...
        #data_dict = baseline_utils.get_data(obs_len, pred_len, use_map, normalize, use_delta, baseline_key, test_features = test_features)
