...
from model.LaneGCN.test import main as main_test
...
class LaneGCN(MotionPrediction):
    def __init__(self, config: Dict, logger: Logger):
        ...
        self.test_folder = self.config.data.test.raw_dir
    ...
    def train(self):
        ...
        subprocess.call(['horovodrun', '-np', '4', 'python', 'model/LaneGCN/train_lanegcn.py',  '--train_dir', self.train_folder, 
        '--val_dir', self.val_folder, '--test_dir', self.test_folder, '--split', 'train', '--obs_len', str(self.obs_len), 
        '--pred_len', str(self.pred_len), '--raw_train_dir', self.train_features_path, '--raw_val_dir', self.val_features_path, 
        '--raw_test_dir', self.test_features_path, '--weight', self.train_cfg.weight, '--resume', self.train_cfg.resume])
    ...
    def validate(self):
        ...
        subprocess.call(['python', 'model/LaneGCN/train_lanegcn.py',  '--train_dir', self.train_folder, 
        '--val_dir', self.val_folder, '--test_dir', self.test_folder, '--split', 'val', '--obs_len', str(self.obs_len), 
        '--pred_len', str(self.pred_len), '--raw_train_dir', self.train_features_path, '--raw_val_dir', self.val_features_path, 
        '--raw_test_dir', self.test_features_path, '--weight', self.val_cfg.weight, '--resume', self.val_cfg.resume])
