...
def _get_multi_predictions(trajectory: np.ndarray, pred_len: int, avg_point_list: Tuple):
    ...
    for i in avg_point_list:
        vx, vy = _get_mean_velocity(trajectory, avg_history_points=i)
...
class ConstantVelocity(MotionPrediction):
    ...
    def get_mean_velocity(self, coords: np.ndarray, avg_history_points: int) -> Tuple[np.ndarray, np.ndarray]:
        """Get mean velocity of the observed trajectory.
        ...
        """
        return _get_mean_velocity(coords, avg_history_points)

    def predict(self, obs_trajectory: np.ndarray, vx: np.ndarray, vy: np.ndarray, pred_len: int) -> np.ndarray:
        """Predict future trajectory given mean velocity.
        ...
        """
        return _predict(obs_trajectory, vx, vy, pred_len)
    ...
    def train(self):
        ...
        :return:
        '''
        print('Constant Velocity does not need training. Done\n')
...
class ConstantVelocityPlanner():
    ...
    def predict(self, trajectory: np.ndarray, pred_len: int, avg_point_list: Tuple):
        '''
        ...
        '''
        return _get_multi_predictions(trajectory, pred_len, avg_point_list)[0]
