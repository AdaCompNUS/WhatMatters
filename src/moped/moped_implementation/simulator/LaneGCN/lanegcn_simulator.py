...
from simulator.LaneGCN import lanegcn
...
class LaneGCN(Simulator):
    WEIGHT_PATH = f'{ROOT}/36.000.ckpt'
    def __init__(self, gpu_device: int = 0):
        ...
        # Below 3 lines are solutions of GPU when running 1 process per pomdp planner
        # print("IS CUDA AVAILABLE: {} with gpus: {}".format(torch.cuda.is_available(), torch.cuda.device_count()))
        # print("Using GPU device: {} with {}".format(gpu_device, torch.cuda.current_device()))
        # gpu_device = torch.cuda.current_device()

        # Setting torch cuda device
        # Fix ordinal bug
        print("LaneGCN Using GPU device: old {} with new {}".format(gpu_device, torch.cuda.current_device()))
        self.gpu_device = torch.cuda.current_device()
        torch.cuda.set_device(self.gpu_device)
        print("LaneGCN Using GPU device: {}".format(self.gpu_device))

        model = importlib.import_module('.lanegcn', package=__package__)
        self.config, _, self.net, _, _, _ = model.get_model()
        # load pretrain model
        ckpt_path = LaneGCN.WEIGHT_PATH
    ...
    def run(self, obs_trajectory):
        # This function now is running only in CPU.
        # To use GPU, change 2 locations:
        # 1. In the get_model() function in lanegcn.py, change to .cuda()
        # 2. In utils(), uncommented  .cuda()
        #print(f"obs shape: {obs_trajectory.shape}")
        data_run= self.preprocess(obs_trajectory)
        data_run = collate_fn(data_run)
        with torch.no_grad():
            output = self.net(data_run)
            results = output["reg"][0].detach().cpu().numpy()
        #print(f"results shape: {results.shape}")
        return results

    def array2dict(self, trajectories: np.ndarray):

        data = self.getTrajectoryFeatures(trajectories)
        data['graph'] = self.getMapFeatures(data)
        graph = modify(to_long(gpu(from_numpy(data['graph']))), self.config['cross_dist'])
        data['graph'] = graph
        return data


    def dict2run(self, data: Dict):
        data_run = []
        new_data = dict()
