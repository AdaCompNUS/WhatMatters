...
from simulator.HiVT.losses import LaplaceNLLLoss
from simulator.HiVT.losses import SoftTargetCrossEntropyLoss
from simulator.HiVT.metrics import ADE
from simulator.HiVT.metrics import FDE
from simulator.HiVT.metrics import MR
from simulator.HiVT.models import GlobalInteractor
from simulator.HiVT.models import LocalEncoder
from simulator.HiVT.models import MLPDecoder
from simulator.HiVT.utils import TemporalData
...
class HiVT(pl.LightningModule):
    ...
    def validation_step(self, data, batch_idx):
        ...
        first_mode_agent = torch.zeros_like(best_mode_agent)
        y_hat_best_agent = y_hat_agent[best_mode_agent, torch.arange(data.num_graphs)]
        y_hat_firstmode_agent = y_hat_agent[first_mode_agent, torch.arange(data.num_graphs)]
        ...
        self.ADE.update(y_hat_firstmode_agent, y_agent)
        self.FDE.update(y_hat_firstmode_agent, y_agent)
        self.MR.update(y_hat_firstmode_agent, y_agent)
        ...
        self.log('val_ADE', self.ADE, prog_bar=True, on_step=False, on_epoch=True, batch_size=y_agent.size(0))
        self.log('val_FDE', self.FDE, prog_bar=True, on_step=False, on_epoch=True, batch_size=y_agent.size(0))
        self.log('val_MR', self.MR, prog_bar=True, on_step=False, on_epoch=True, batch_size=y_agent.size(0))
