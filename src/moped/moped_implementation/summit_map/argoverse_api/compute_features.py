...
from baseline_config import RAW_DATA_FORMAT, _FEATURES_SMALL_SIZE

from map_features_utils import MapFeaturesUtils
from social_features_utils import SocialFeaturesUtils
...
def parse_arguments() -> Any:
    ...
    parser.add_argument(
        "--data_dir",
        default="/home/cunjun/p3_catkin_ws_new/src/moped/moped_implementation/summit_map/data/summit/",
        type=str,
        help="Directory where the sequences (csv files) are saved",
    )
    parser.add_argument(
        "--feature_dir",
        default="/home/cunjun/p3_catkin_ws_new/src/moped/moped_implementation/summit_map/output/",
        type=str,
...
def load_seq_save_features(
    ...
    data_df.to_csv(
        f"{save_dir}/forecasting_features_{args.mode}_{start_idx}_{start_idx + args.batch_size}.csv"
    )
...
def merge_saved_features(batch_save_dir: str) -> None:
    ...
    for feature_file in feature_files:
        if not feature_file.endswith(".csv") or args.mode not in feature_file:
            continue
    ...
    all_features_df.to_csv(
        f"{args.feature_dir}/forecasting_features_{args.mode}.csv")
