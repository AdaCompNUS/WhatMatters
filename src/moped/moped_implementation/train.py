...
def construct_model(cfg, logger):
    ...
    #model = getattr(model_class, model_name)(config=cfg, logger=logger)
    return model
...
def construct_dataset(config):
    ...
    cfg: Config class
    '''
    if config.data.dataset_type == "SummitDataset" and config.model.type != "LaneGCN" and config.model.type != "HiVT":
    ...
    elif config.data.dataset_type == "ArgoverseDataset":
