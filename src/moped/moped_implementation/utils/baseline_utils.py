...
def get_data_for_knn_planner(args: Any, baseline_key: str
    ...
    if isinstance(args.test_features, str) and args.test_features or isinstance(args.test_features, pd.DataFrame):
        print("Loading Test data ...")
        test_input, test_output, test_df = load_and_preprocess_data_for_knn_planner(
            input_features,
            output_features,
            args,
            args.test_features,
...
def load_and_preprocess_data_for_knn_planner(
        input_features: List[str],
        output_features: List[str],
        args: Any,
        feature_file: Union[str, pd.DataFrame],
        mode: str = "train",
    ...
    elif isinstance(feature_file, pd.DataFrame):
        df = feature_file
    else:
        raise ValueError("Feature file is not expected type")
    ...
    if args.use_delta:
        ...
        if args.use_map and mode == "test":
            ...
            for candidate_nt_dist_i in candidate_nt_distances:
                curr_reference = []
                for curr_candidate_nt in candidate_nt_dist_i:
                    curr_candidate_reference = np.full(shape = (curr_candidate_nt.shape[1],), fill_value=None)
                    curr_reference.append(curr_candidate_reference.squeeze())
            ...
            df["DELTA_REFERENCE"] = np.full(shape = (input_features_data.shape[0], input_features_data.shape[2]),
...
def merge_saved_traj(batched_dir: str, merged_file_path: str) -> dict:
