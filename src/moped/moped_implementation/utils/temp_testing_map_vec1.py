...
def get_candidate_centerlines_for_trajectory(
    ...
    Args:
        xy: Trajectory coordinates, [n, obs, 2]
        city_name: City name,
        avm: Argoverse map_api instance,
    ...
    curr_lane_candidates = [avm.get_lane_ids_in_xy_bbox(
        traj[-1, 0], traj[-1, 1], city_name, manhattan_thresnold) for traj in xy]
    curr_lane_candidates = [lane for sublanes in curr_lane_candidates for lane in sublanes]
    ...
    obs_pred_lanes_with_scores = [sort_lanes_based_on_point_in_polygon_score(
        obs_pred_lanes, traj, city_name) for traj in xy]
    ...
    candidate_centerlines = [get_heuristic_centerlines_for_test_set(
            obs_pred_lanes, traj, city_name, max_candidates, scores) for
        traj, (obs_pred_lanes, scores) in zip(xy, obs_pred_lanes_with_scores)]
...
def compute_map_features_speedup(agent_track: np.ndarray, obs_len: int,
    ...
    agent_track : Data for the agent track, shape [n, obs_len, 2]
    obs_len: 20
