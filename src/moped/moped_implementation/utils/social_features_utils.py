...
class SocialFeaturesUtils:
    ...
    def compute_social_features(
        ...
        min_distance_front_and_back_obs = self.get_min_distance_front_and_back(
            ...
            raw_data_format,
            viz=False)
        min_distance_front_and_back_obs_vec = SocialFeaturesUtilsVectorize().get_min_distance_front_and_back_vec(
            agent_track_obs, social_tracks_obs, obs_len, raw_data_format
        )

        assert np.isclose(min_distance_front_and_back_obs_vec, min_distance_front_and_back_obs).all()

        # Get number of neighbors
        num_neighbors_obs = self.get_num_neighbors(agent_track_obs,
                                                   social_tracks_obs, obs_len,
                                                   raw_data_format)
        num_neighbors_obs_vec = SocialFeaturesUtilsVectorize().get_num_neighbors_vec(agent_track_obs, social_tracks_obs, obs_len, raw_data_format)
        assert np.isclose(num_neighbors_obs_vec, num_neighbors_obs).all()
